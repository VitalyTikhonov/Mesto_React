...
const SignupInputSet = memo(function SignupInputSet(props) {
  ...
  return (
    <>
      ...
      <input type="email" name="email" value={inputStateValues.email} id="email" className="popup__input" onChange={handleFieldChange} onInput={handleFieldInput} placeholder="Адрес электронной почты" required minLength={2} maxLength={30} />
      <span className="popup__error" id="email-error" />
      <input type="password" name="password" value={inputStateValues.password} id="password" className="popup__input" onChange={handleFieldChange} onInput={handleFieldInput} placeholder="Пароль" required minLength={2} maxLength={30} />
